...
class KNN:
    ...
    def weighted_majority_vote(self):
        ...
        for i in range(self.K):
            try:  # 만약 가중치가 1/0이 되서 무한대가 된다면 그냥 그 점에 해당하는 y값과 같다는 것이므로 그것을 return
                weight_sum += 1 / self.nearest[i][0]  # 값 환산을 위해 더해준다
            except ZeroDivisionError:  # 만약 /0 이 발생한다면
                return self.y_name[self.y[self.nearest[i][1]]]  # 그것은 그 점과 같으므로 그냥 그점에 해당하는 y의 이름을 바로 return 해준다
        for i in range(self.K):
            try:  # 만약 가중치가 1/0이 되서 무한대가 된다면 그냥 그 점에 해당하는 y값과 같다는 것이므로 그것을 return
